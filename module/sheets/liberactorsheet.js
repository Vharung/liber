export class LiberActorSheet extends ActorSheet {
    ...
    _onAvantageRace(event){
        var clanliste =this.actor.data.data.clan;
        var avantagerace="";
        var armureperso=this.actor.data.data.protection;
        this.actor.update({"acrobatie": 0}); this.actor.update({"agilites": 0}); this.actor.update({"alchimie": 0}); this.actor.update({"apprentissage": 0}); this.actor.update({"hast": 0}); this.actor.update({"cc": 0}); this.actor.update({"lancer": 0}); this.actor.update({"melee": 0}); this.actor.update({"tir": 0}); this.actor.update({"art": 0}); this.actor.update({"assassinat": 0}); this.actor.update({"baton": 0}); this.actor.update({"bouclier": 0}); this.actor.update({"bricolage": 0}); this.actor.update({"presence": 0}); this.actor.update({"chercher": 0}); this.actor.update({"commander": 0}); this.actor.update({"concentration": 0}); this.actor.update({"nature": 0}); this.actor.update({"peuples": 0}); this.actor.update({"religions": 0}); this.actor.update({"geographique": 0}); this.actor.update({"rue": 0}); this.actor.update({"heretiques": 0}); this.actor.update({"combat": 0}); this.actor.update({"commerce": 0}); this.actor.update({"crochetage": 0}); this.actor.update({"discretion": 0}); this.actor.update({"dexterite": 0}); this.actor.update({"detection": 0}); this.actor.update({"dissimulation": 0}); this.actor.update({"dressage": 0}); this.actor.update({"ennemi": 0}); this.actor.update({"equilibre": 0}); this.actor.update({"equitation": 0}); this.actor.update({"escroquerie": 0}); this.actor.update({"esquiver": 0}); this.actor.update({"puissance": 0}); this.actor.update({"astuce": 0}); this.actor.update({"peur": 0}); this.actor.update({"joueur": 0}); this.actor.update({"maitrise": 0}); this.actor.update({"natation": 0}); this.actor.update({"navigation": 0}); this.actor.update({"orientation": 0}); this.actor.update({"persuasion": 0}); this.actor.update({"pister": 0}); this.actor.update({"prophetie": 0}); this.actor.update({"secours": 0}); this.actor.update({"resistance": 0}); this.actor.update({"psychologue": 0}); this.actor.update({"medecine": 0}); this.actor.update({"survie": 0}); this.actor.update({"tueur": 0}); this.actor.update({"objet": 0}); this.actor.update({"veterinaire": 0}); this.actor.update({"vigilance": 0}); this.actor.update({"vise": "0"});
        var raceliste=this.actor.data.data.race;
        ...
        this.actor.update({'data.bonusrace': avantagerace});
    }

    _onName(event){
        var race = this.actor.data.data.race;
        var sexe = this.actor.data.data.sexe;
        if(race=="Dragon"){
